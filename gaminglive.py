...
def generate_msg(message):
    structure = {"user": {"nick": CONST_USERNAME, "isHost": False,
                          "isRegistered": True}, "message": message}
...
def on_gaminglive_open(wsptr):
    ws.send(
        generate_msg("glbot has connected! You should see this message in chat."))

wss_url = "ws://api.gaminglive.tv/chat/%s?nick=%s&authToken=%s" % (
    CONST_BROADCASTER, CONST_USERNAME, CONST_TOKEN)
print(
    "=== Starting up: gaminglivebot/0.0.1\n=== github.com/dillpickle/gaminglive for updates")
if CONST_USERNAME == "_CHANGE_ME_":
    sys.exit("==== Please update your configuration.")
